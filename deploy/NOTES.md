...
## Deploy log: 2026-08-19T14:11Z
- Statics at 430afe7 (newsletter form + subscribed/unavailable pages): parity dry-run zero transfers, all pages 200, R-02 live-grep clean. SUBSCRIBE SERVICE ENABLED on Austins "push everything to production": /opt/austinfiala/service + data dir 700, unit installed (kernel sandbox, IPAddressDeny=any, loopback bind confirmed via ss), ADMIN_TOKEN in subscribe.env mode 600 (copy handed to Austin via scratchpad). Caddy: live file read, backed up, handle /subscribe + handle_errors added, validate passed, RELOADED, four sites 200. Smoke: real post 303 + row in loopback export; honeypot 303-identical + zero rows; export.csv unroutable from outside (404); rate limit engaged (429s, proving X-Real-IP trust); service stopped = plain unavailable page with mailto, restarted; probe rows deleted, table back to 0. Real-Chrome render verified (form present, fine print live). Box memory after fifth service: ~200MB available + swap.
